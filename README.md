...
provisioned end-to-end by Terraform. Built for IT-342 (cloud development).
...
│                   by EC2 user-data inside the Terraform launch templates.
├── infra/          Terraform IaC for the entire AWS stack (VPC, ALBs, ASGs,
│                   RDS, S3, CloudFront, Secrets Manager, WAF, alarms, SES).
├── .github/        CI workflows: terraform plan/apply, app deploy via OIDC,
│                   syntax checks.
└── docs/           Architecture, AWS-services rationale, deployment guide,
                    cost estimate, security model, runbook.
```

## AWS services used

VPC · EC2 + Auto Scaling Groups · Application Load Balancer (×2) · RDS MySQL
Multi-AZ · Lambda · EventBridge · SQS · SNS · Secrets Manager · CloudFront
· S3 · ACM · Route 53 · CloudWatch (Logs / Metrics / Alarms / Dashboard)
· AWS WAF · SES · IAM · KMS · Systems Manager (Session Manager + Parameter
Store via SSM AMI lookup) · GitHub OIDC.
...
# 2. Provision (~15 min — RDS Multi-AZ is the slow part)
terraform init
terraform apply

# 3. Seed the DB (see docs/deployment.md for the SSM Session Manager flow)

# 4. Visit
open "$(terraform output -raw public_url)"
```
...
| [docs/deployment.md](docs/deployment.md) | Step-by-step deploy from zero |
...
- **No secrets in `.env`**. `.env.example` documents the shape; production
  pulls from Secrets Manager. The setup scripts also support a Secrets-
  Manager-driven boot for ad-hoc EC2 deploys outside Terraform.
- **Idempotent everything**: setup scripts and Terraform can be re-run
  without breaking state.
- **Tagged everything**: `Project`, `Environment`, `Owner`, `ManagedBy`
  on every resource via Terraform `default_tags`.
